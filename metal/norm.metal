...
kernel void kernel_rms_norm_fuse_impl(
    ...
    for (int i00 = tpitg.x; i00 < args.ne00_t; i00 += ntg.x) {
        ...
        if (F == 2) {
            T value = (x[i00]*scale)*f0[i00];
            if (ROUND_BF16) {
                uint4 bits = as_type<uint4>(value);
                const bool4 finite = (bits & 0x7f800000u) != 0x7f800000u;
                bits += select(uint4(0), 0x7fffu + ((bits >> 16u) & 1u), finite);
                value = as_type<T>(bits & 0xffff0000u);
            }
            y[i00] = value;
        }
        if (F == 3) {
...
template [[host_name("kernel_dsv41_rms_norm_mul_bf16")]] kernel kernel_rms_norm_fuse_t kernel_rms_norm_fuse_impl<float4, 2, true>;
